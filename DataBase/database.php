...
class DataBase {
    ...
    {
        $this->db_connection = self::getDbConnection($db_name);
    }
    ...
    public function get_limit($table , $condicao = 'processamento = 0', $order = 'asc', $db_name = null , $params = '*')
    {
        $dbconnection = $this->getDbConnection($db_name);
        
        $sql = "SELECT $params FROM $table WHERE $condicao order by id $order limit 1";
        // echo $sql;
        $sth = $dbconnection->prepare($sql);
        $resultQuery = $sth->execute();
        if (!$resultQuery) return false;
    ...
    public function get($table , $condicao = 'processamento = 0', $db_name = null , $params = null){
        ...
        if (!$params) {
            if ($condicao) {
                $sql = "SELECT * FROM $table WHERE $condicao limit 64";
                // echo $sql;
                $sth = $dbconnection->prepare($sql);
                $resultQuery = $sth->execute();
            ...
            $sql = "SELECT * FROM $table limit 64";
            $sth = $dbconnection->prepare($sql);
            $resultQuery = $sth->execute();
        ...
        if ($condicao) {
            $sql = "SELECT $params FROM $table WHERE $condicao limit 64";
            // echo $sql . '<br>';
        ...
        $sql = "SELECT $params FROM $table limit 64";
        $sth = $dbconnection->prepare($sql);
        $resultQuery = $sth->execute();
        if (!$resultQuery) return false;
        return $sth->fetchAll(PDO::FETCH_ASSOC);
